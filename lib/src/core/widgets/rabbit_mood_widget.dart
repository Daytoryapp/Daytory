...
class RabbitMoodWidget extends StatelessWidget {
  const RabbitMoodWidget({super.key, required this.moodScore, this.size = 36.0, this.isMale = false});
  final int moodScore;
  final double size;
  final bool isMale;
  ...
  Widget build(BuildContext context) {
    final clampedMood = moodScore.clamp(1, 5);
    return SizedBox(
      width: size,
      height: size,
      child: CustomPaint(
        painter: isMale
            ? _FoxPainter(clampedMood)
            : _RabbitPainter(clampedMood),
      ),
    );
  }
}
...
class _FoxPainter extends CustomPainter {
  const _FoxPainter(this.mood);
  final int mood;

  static const _faceColor    = Color(0xFFFF9E4A);
  static const _borderColor  = Color(0xFFD96B1A);
  static const _earInnerColor = Color(0xFF8B3A0F);
  static const _muzzleColor  = Color(0xFFFFF5E0);
  static const _noseColor    = Color(0xFF2A1A0A);
  static const _inkColor     = Color(0xFF1E1A1D);
  ...
  void _drawEars(Canvas canvas, double cx, double cy, double fr) {
    final face  = Paint()..color = _faceColor;
    final border = Paint()
      ..color = _borderColor
      ..style = PaintingStyle.stroke
      ..strokeWidth = fr * 0.09;
    final inner = Paint()..color = _earInnerColor;
    ...
    for (final side in [-1.0, 1.0]) {
      ...
      canvas.save();
      canvas.translate(ex, ey);
      canvas.rotate(side * 0.25);
      final rect   = Rect.fromCenter(center: Offset.zero, width: fr * 0.38, height: fr * 1.00);
      final inner2 = Rect.fromCenter(center: const Offset(0, 4), width: fr * 0.18, height: fr * 0.65);
  ...
  void _drawMuzzle(Canvas canvas, double cx, double cy, double fr) {
    canvas.drawOval(
      Rect.fromCenter(center: Offset(cx, cy + fr * 0.22), width: fr * 0.80, height: fr * 0.48),
      Paint()..color = _muzzleColor,
    );
  }

  void _drawBlush(Canvas canvas, double cx, double cy, double fr) {
    final p = Paint()..color = const Color(0xFFFF7020).withAlpha(mood == 5 ? 80 : 55);
    canvas.drawOval(Rect.fromCenter(center: Offset(cx - fr * 0.60, cy + fr * 0.27), width: fr * 0.50, height: fr * 0.26), p);
    canvas.drawOval(Rect.fromCenter(center: Offset(cx + fr * 0.60, cy + fr * 0.27), width: fr * 0.50, height: fr * 0.26), p);
  }
  ...
  void _drawNose(Canvas canvas, double cx, double cy, double fr) {
    canvas.drawOval(
      Rect.fromCenter(center: Offset(cx, cy + fr * 0.05), width: fr * 0.19, height: fr * 0.13),
      Paint()..color = _noseColor,
    );
  }
      ...
      case 5: // big smile + fill
        ...
        canvas.drawPath(
          Path()
            ...
            ..quadraticBezierTo(cx, my + mw * 1.2, cx + mw * 1.2, my - mw * 0.1)
            ..close(),
          Paint()..color = const Color(0xFFFF7020).withAlpha(70),
        );
        canvas.drawPath(smilePath, stroke);
    }
  }
  ...
  @override
  bool shouldRepaint(_FoxPainter old) => old.mood != mood;
}
